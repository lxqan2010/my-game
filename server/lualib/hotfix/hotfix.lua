...
local function check_updated(new_obj, old_obj, name, deep)
    local signature = string.format("new(%s) old(%s)",
        _tostring(new_obj), _tostring(old_obj))
...
function update_func(new_func, old_func, name, deep)
    ...
    if protected[old_func] then
        M.log_debug(string.format("%s%s is protected !", deep, name))
        return
    end
    ...
    local function log_dbg(name, from, to)
        M.log_debug(string.format("%ssetupvalue %s: (%s) -> (%s)",
            deep, name, _tostring(from), _tostring(to)))
    end
...
function update_table(new_table, old_table, name, deep, replace)
    ...
    if replace then
        ...
        table.merge(old_table, new_table)
    else
        deep = deep .. "  "
        ...
        for key, value in pairs(new_table) do
            ...
            if type_value ~= type(old_value) then
                ...
                M.log_debug(string.format("%sUpdate field %s: (%s) -> (%s)",
                    deep, key, _tostring(old_value), _tostring(value)))
...
local function update_loaded_module(new_obj, module_name, replace)
    assert(nil ~= new_obj)
    ...
    if new_type == old_type then
        if "table" == new_type then
            update_table(new_obj, old_obj, module_name, "", replace)
            return
        end
    ...
    M.log_debug(string.format("Directly replace module: old(%s) -> new(%s)",
        _tostring(old_obj), _tostring(new_obj)))
...
function M.hotfix_module(module_name, replace)
    assert("string" == type(module_name))
    ...
    --]]
    local func = assert(loadfile(file_path))
    ...
    updated_func_map = {}
    do
        update_loaded_module(obj, module_name, replace)

        if next(updated_func_map) then
            replaced_obj = {}
            replace_functions(_G)
            --replace_functions(package.loaded[module_name])
...
-- Add object to protect.
-- Example: add_protect(table)
function M.add_one_protect(object)
