...
local type, table, string, _tostring, tonumber = type, table, string, _tostring, tonumber
local select = select
local error = error
...
local function LOG_MSG(self, level, fmt, ...)
    ...
    if f_type == 'string' then
        if select('#', ...) > 0 then
            local status, msg = xpcall(format, debug.traceback, fmt, ...)
            if status then
